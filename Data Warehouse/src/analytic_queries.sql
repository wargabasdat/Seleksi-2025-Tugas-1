...
SELECT y.year_id AS year, ROUND(AVG(f.rating)::numeric, 2) AS average_rating
...
SELECT d.director_name, y.year_id AS year, ROUND(AVG(r.rating)::numeric, 2) AS avg_rating
FROM fact_director_credit f
JOIN dim_director d ON f.director_id = d.director_id
JOIN dim_year y ON f.year_id = y.year_id
...
SELECT d.director_name, ROUND(AVG(r.rating)::numeric, 2) AS avg_rating
FROM fact_director_credit f
JOIN dim_director d ON f.director_id = d.director_id
JOIN fact_videogame_rating r ON f.videogame_id = r.videogame_id AND f.year_id = r.year_id
