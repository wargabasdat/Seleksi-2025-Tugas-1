...
import json
import os
# import re # Tidak lagi dibutuhkan
...
def create_all_tables(conn):
    ...
    ddl_commands = [
        ...
        # MODIFIED: Tabel 'game_media' sesuai diagram (sebelumnya 'critics')
        """CREATE TABLE IF NOT EXISTS game_media ( critic_id SERIAL PRIMARY KEY, name VARCHAR(100) UNIQUE NOT NULL, website VARCHAR(255) );""",
        
        # MODIFIED: Tabel 'games' disederhanakan tanpa foreign key langsung ke developer/publisher
        """
        ...
        # NEW: Tabel perantara untuk hubungan many-to-many games dan developers
        """
        ...
        """,
        # NEW: Tabel perantara untuk hubungan many-to-many games dan publishers
        """
        ...
        # Tabel lain yang bergantung pada tabel master
        # MODIFIED: FK ke game_media(critic_id)
        ...
        # MODIFIED: Menghapus publisher_id dari regional_releases sesuai diagram
    ...
    try:
        with conn.cursor() as cur:
            for command in ddl_commands:
                cur.execute(command)
        conn.commit()
        print("All tables created or updated successfully according to the new schema.")
...
def load_data_to_db(conn, data):
    try:
        with conn.cursor() as cur:
            for record in data:
                # 1. Insert data ke tabel master (developers, publishers) dan dapatkan ID
                ...
                # 2. MODIFIED: Insert ke tabel 'games' tanpa developer/publisher ID
                cur.execute(
                    """
                ...
                # 3. NEW: Insert relasi ke tabel perantara
                if developer_id:
                    cur.execute(
                ...
                # 4. Insert relasi genre (tetap sama)
                if record.get('genre'):
...
def main():
    ...
    json_file = os.path.join(script_dir, "data", "list_games_ps2.json")
